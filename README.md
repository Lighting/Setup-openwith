...
- При обнаружении Cool reader 3, скрипт предложит создать новую системную тему на основе выбранной текущей темы в настройках Персонализация с замененным виджетом "Поиск" в верхней панели управления на новый "fb2" для быстрого переключения программы по умолчанию открывающую .fb2 файлы (Coor reader 3 или FB Reader/PB Reader). Поддерживается как FB Reader (fbreader.app) так и PB Reader (eink-reader.app). Для изменения интерфейса нужно выбрать новую тему OpenWith в Настройки->Персонализация->Тема. Поиском можно продолжить пользоваться через Библиотеку.
