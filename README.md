...
- При обнаружении Cool reader 3, скрипт предложит создать новую системную тему на основе текущей темы в настройках Персонализация (по умолчанию Line) с замененным виджетом "Поиск" в верхней панели управления на новый "fb2" для быстрого переключения программы по умолчанию открывающую .fb2 файлы (Coor reader 3 или FB Reader/PB Reader). Альтернативе Coor reader 3 используется встроенный просмоторщик FB Reader (fbreader.app) или PB Reader (eink-reader.app). Для изменения интерфейса нужно выбрать новую тему заканчивающуюся на OpenWith в Настройки->Персонализация->Тема. Поиском можно продолжить пользоваться через Библиотеку.
